...
export default function PatientPortal() {
  ...
  useEffect(() => {
    ...
    // If already loaded, just initialize and render
    if (window.google) {
      initAndRenderGoogle();
      return;
    }
    ...
    if (existing) {
      existing.addEventListener("load", initAndRenderGoogle);
      return () => existing.removeEventListener("load", initAndRenderGoogle);
    }
    const script = document.createElement("script");
    script.src = "https://accounts.google.com/gsi/client";
    script.async = true;
    script.defer = true;
    script.onload = initAndRenderGoogle;
    document.head.appendChild(script);
  }, [initAndRenderGoogle]);

  // Re-render button when the ref div becomes available (mode switches)
  useEffect(() => {
